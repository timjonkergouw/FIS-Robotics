...
const PrismaticBurst: React.FC<PrismaticBurstProps> = ({
    ...
    useEffect(() => {
        ...
        gradTexRef.current = gradientTex as unknown as Texture;
        ...
        programRef.current = program as unknown as Program;

        const triangle = new Triangle(gl);
        const mesh = new Mesh(gl, { geometry: triangle, program });
        triRef.current = triangle as unknown as Triangle;
        meshRef.current = mesh as unknown as Mesh;

        const resize = () => {
            const w = container.clientWidth || 1;
            const h = container.clientHeight || 1;
            (renderer as any).setSize(w, h);
            (program as any).uniforms.uResolution.value = [gl.drawingBufferWidth, gl.drawingBufferHeight];
        };
        ...
        const update = (now: number) => {
            ...
            (program as any).uniforms.uMouse.value = sm;
            (program as any).uniforms.uTime.value = accumTime;

            (renderer as any).render({ scene: meshRef.current });
            raf = requestAnimationFrame(update);
        };
        raf = requestAnimationFrame(update);
        ...
        return () => {
            ...
            try {
                const glCtx = (rendererRef.current as any)?.gl;
                if (glCtx && (gradTexRef.current as any)?.texture) {
                    glCtx.deleteTexture((gradTexRef.current as any).texture);
                }
            } catch { }
    ...
    useEffect(() => {
        const canvas = (rendererRef.current as any)?.gl?.canvas as HTMLCanvasElement | undefined;
        if (canvas) {
            (canvas.style as any).mixBlendMode = mixBlendMode && mixBlendMode !== 'none' ? (mixBlendMode as any) : '';
        }
    }, [mixBlendMode]);

    useEffect(() => {
        const program = programRef.current as any;
        const renderer = rendererRef.current as any;
        const gradTex = gradTexRef.current as any;
        ...
        const ox = toPx((offset as any)?.x);
        const oy = toPx((offset as any)?.y);
        program.uniforms.uOffset.value = [ox, oy];
        program.uniforms.uRayCount.value = Math.max(0, Math.floor((rayCount as any) ?? 0));
        ...
        if (Array.isArray(colors) && colors.length > 0) {
            ...
            for (let i = 0; i < count; i++) {
                const [r, g, b] = hexToRgb01(capped[i] as string);
